    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> EditProfile(EditProfileViewModel model)
        {
            ...
            if (model.ProfileImage != null && model.ProfileImage.Length > 0)
            {
                ...
                {
                    var oldPath = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "images", "profiles", user.ProfileImageUrl);
        ...
        public async Task<IActionResult> RemoveProfileImage()
        {
            ...
            if (!string.IsNullOrEmpty(user.ProfileImageUrl) && user.ProfileImageUrl != "default.png")
            {
                var imagePath = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "images", "profiles", user.ProfileImageUrl);
                if (System.IO.File.Exists(imagePath))
                {
                    System.IO.File.Delete(imagePath);
                }
            }
            // إعادة تعيين الصورة إلى الافتراضية
